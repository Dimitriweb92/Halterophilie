...
class ArticleManager
{
    ...
    public function listArticle(){
        ...
        $get = $this->db->query("SELECT a.*,
          u.idadmin,u.thelogin,u.thename, c.titre, c.id
          FROM article a INNER JOIN admin u 
            ON a.admin_idadmin = u.idadmin INNER JOIN rubrique c ON a.admin_idadmin = c.id
    ...
    public function listArticleCateg(int $id){
        ...
        $get = $this->db->query("SELECT a.*,
          u.idadmin,u.thelogin,u.thename, c.titre, c.id
          FROM article a INNER JOIN admin u 
            ON a.authoridauthor = u.idadmin INNER JOIN rubrique c ON a.admin_idadmin = c.id
          WHERE c.id ='$id';");
    ...
    public function createArticle(Article $datas){
        ...
        if($datas->getAdminIdadmin()==$_SESSION['idauthor']){
            ...
            $sql = "INSERT INTO article (thetitle,thetext,thedate,admin_idadmin,rubrique_rubriqueid) VALUES (?,?,?,?,?)";
    ...
    public function updateArticle(Article $newDatas, int $getIdArticle){
        ...
        if($newDatas->getAdminIdadmin()==$_SESSION['idauthor']){
            ...
            if($newDatas->getIdarticle()==$getIdArticle){
                ...
                $sql = "UPDATE article SET thetitle=?, thetext=?, thedate=?, rubrique_rubriqueid=?  WHERE idarticle=?";
